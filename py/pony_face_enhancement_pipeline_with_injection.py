...
class PonyFaceEnhancementPipelineWithInjection:
    ...
    @classmethod
    def INPUT_TYPES(s, exec_context: execution_context.ExecutionContext=None):
    ...
    def execute(self, image, model, positive, negative, vae, cfg, face_bbox_model, face_segm_model, 
        ...
        if enable_noise_injection == "enable":
            ...
            enhanced_latent_tuple = common_ksampler(exec_context, model, actual_seed, steps, cfg, sampler_name, scheduler, positive, negative, face_latent, denoise=denoise)
            enhanced_latent = enhanced_latent_tuple[0]
