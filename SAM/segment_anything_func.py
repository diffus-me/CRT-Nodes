...
def load_sam_model(exec_context: execution_context.ExecutionContext, model_name):
    sam_checkpoint_path = get_local_filepath(exec_context,
...
def get_local_filepath(exec_context: execution_context.ExecutionContext, url, dirname, local_file_name=None):
    if not local_file_name:
        parsed_url = urlparse(url)
        local_file_name = os.path.basename(parsed_url.path)

    destination = folder_paths.get_full_path(exec_context, dirname, local_file_name)
    if destination:
...
def load_groundingdino_model(exec_context: execution_context.ExecutionContext, model_name):
    from local_groundingdino.util.utils import clean_state_dict as local_groundingdino_clean_state_dict
    from local_groundingdino.util.slconfig import SLConfig as local_groundingdino_SLConfig
    from local_groundingdino.models import build_model as local_groundingdino_build_model
    dino_model_args = local_groundingdino_SLConfig.fromfile(
        get_local_filepath(exec_context,
